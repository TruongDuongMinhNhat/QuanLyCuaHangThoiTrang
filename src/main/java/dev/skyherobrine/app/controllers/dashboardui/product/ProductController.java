...
public class ProductController implements ActionListener, MouseListener, KeyListener, TableActionEvent, TableModelListener {
    ...
    private ArrayList<ChiTietPhienBanSanPham> listCTPBSP = new ArrayList<>();
...
//  Load data từ CSDL lên table
    ...
    private SanPham layDataThem() {
        ...
        float ptl = 0;
        if(sanPhamUI.getTxtPhamTramLoi().getText().toString().matches("\\d+")){
            ptl = Float.parseFloat(sanPhamUI.getTxtPhamTramLoi().getText().toString());
            if(ptl<0){
